...
| Módulo | Por qué |
|--------|---------|
| `lib/ranking/elo.ts` | Singles Elo por unit (K, margen de games en Sets, orden cronológico, inactividad Games >21 días Lima, Sets nunca inactivo, no doble conteo) |
...
| `lib/sessions/financier.ts` | Reasignar financiador: solo admin, y solo a un miembro |
...
| `lib/avatar/toStored.ts` | ≤ 500 KB se guarda sin `sharp` (Vercel no carga libvips) |
