...
9. **Admin de app** (`User.isAdmin`) ≠ owner de grupo. Privilegios: editar/borrar fechas, cambiar quién pagó (financiador), cambiar/quitar recoge bolas después de aplicado, cambiar RSVP de miembros en fechas **aún abiertas**, saldar deudas pasadas, y el link de Google Calendar del grupo. Fechas Pasadas: asistencia inmutable (también para admin). No implica gestionar ajustes del grupo (nombre, invite, borrar).
...
- Setup: `web/docs/SETUP.md` (incluye VAPID). Ranking = Elo singles por unit Games/Sets (`web/src/lib/ranking/elo.ts`; Elo.S pondera margen de games); resumen de fecha (`sessionResumen.ts`); ficha de jugador Games|Sets desde Ranking (`playerStats.ts`). Sin ranking de dobles en UI. Tests: `cd web && npm test`. Auditoría Elo vs DB: `cd web && npm run audit:elo`.
- Resultados: Games con ganador obligatorio + Servidor opcional; Sets pueden quedar En curso; soft-delete + historial en modal; Resumen Games (Elo.G) + Resumen Sets si hubo sets (Elo.S); UX optimista. Sin chat de fecha. Deudas: solo Fechas pasadas; saldo entre dos personas (se compensan); Te deben/Debes, sheet Pagar del saldo (Yape/Plin + WhatsApp), “Ya pagué” + push, Saldar saldo cierra ambos lados (`settledAsNet`), Historial de saldadas (últimas 5; Ver todo el historial carga el resto). Al entrar: modal si el saldo incluye deudas `open` de Fechas con más de 7 días (Lima). Perfil: celular de cobro en `/ajustes`.
