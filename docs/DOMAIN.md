...
**Ficha de jugador (UI, Ranking):** en Ranking Singles, tocar avatar/nombre abre un sheet con estadísticas del **ladder activo** (pestaña Games → Elo.G; pestaña Sets → Elo.S). On-read (`web/src/lib/ranking/playerStats.ts` + `unit`); sin ratings persistidos. Incluye: historial de Elo de esa unit (gráfico: este mes / 30 días / desde el inicio; eje X compartido: **Inicio en 1000** + **un punto por cada Fecha pasada del grupo**; si no jugó esa Fecha, el Elo se arrastra), Elo máximo, % ganados, racha más larga (continua entre fechas), rival más jugado (W–L), mayor +Elo en una fecha, asistencia (`going` / fechas elegibles pasadas desde `joinedAt`, respetando `allowedUserIds`), tendencia de las últimas 3 fechas con Voy (W–L + Δ Elo de esa unit). En Games, si hay ≥10 con `serverSide`— % ganados sacando vs recibiendo. Sets no usan servidor; si no hay Sets jugados, las KPIs de matches quedan vacías (asistencia sí puede mostrarse).
